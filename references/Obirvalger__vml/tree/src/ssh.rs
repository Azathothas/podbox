...
impl Ssh {
    pub fn new(config: &ConfigSsh, config_net: &Option<ConfigNet>) -> Option<Ssh> {
        let config_net = if let Some(cn) = &config_net { cn } else { return None };
