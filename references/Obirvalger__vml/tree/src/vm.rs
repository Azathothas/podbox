        ...
        match images::find(&images_dirs, image_name) {
            Ok(image_path) => {
                if config.images.update_on_create
                    && let Some(image) = available_images.get(image_name)
                    && image.outdate()
                {
...
impl VM {
    ...
    pub fn from_config_vm_config<S: AsRef<str>>(
        ...
        let mut net_config = match (&vm_config.net, &config.default.net) {
            (None, Some(_)) => config.default.net.to_owned(),
            (Some(vm_net), Some(default_net)) => Some(vm_net.to_owned().updated(default_net)),
            _ => vm_config.net.to_owned(),
        };
        if let Some(ConfigNet::Tap { nameservers: ref mut nameservers @ None, .. }) = net_config {
    ...
    pub fn start<S: AsRef<OsStr>>(
        ...
        if let Some(net) = &self.net {
            match net {
                ...
                Net::Tap { address, nameservers, tap, mac, .. } => {
                    ...
                    let mac =
                        if let Some(mac) = &mac { mac.to_string() } else { get_random_mac() };
    ...
    pub fn info(&self) -> BTreeMap<&'static str, String> {
        ...
        if let Some(net) = &self.net {
            match net {
                Net::None => {
                    info.insert("network", "none".to_string());
                }
                Net::User => {
