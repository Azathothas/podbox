...
if ($os -eq 'windows') {
    ...
    try {
        ...
        try {
            ...
            $minor = if ($null -ne $cv.CurrentMinorVersionNumber) {
                $cv.CurrentMinorVersionNumber
            } else {
                0
            }
            $distroVer = ('{0}.{1}.{2}' -f
                $cv.CurrentMajorVersionNumber, $minor, $cv.CurrentBuildNumber)
            if ($null -ne $cv.UBR) { $distroVer = ('{0}.{1}' -f $distroVer, $cv.UBR) }
    ...
    foreach ($dir in $fallbackDirs) {
        foreach ($ext in $exeExts) {
            ...
            try {
                ...
                # A PATH entry can be an execution alias or protected
                # directory. Inaccessible candidates are not usable tools.
                continue
            }
        }
    }
