...
if ($templatesExempt) {
    $exempt = '^(references/|docs/templates/|dotfiles/|bootstrap/|scripts/common/check-placeholders\.(sh|ps1))'
}
else {
    $exempt = '^(references/|dotfiles/|scripts/common/check-placeholders\.(sh|ps1))'
}
...
foreach ($rel in $files) {
    ...
    foreach ($line in ($text -split "`r?`n")) {
        ...
        $braceFile = ($rel -cmatch '\.(md|txt|toml|yaml|yml|json|ini|cfg|conf)$' -or
                      $rel -cmatch '(^|/)LICENSE$')
        if ($braceFile -and $line -match '\{\{' -and $line -notmatch '\$\{\{' -and
            $line -cnotmatch '\{\{ *[a-z.]') {
