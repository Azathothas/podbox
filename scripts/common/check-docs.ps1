...
$all = @($tracked + $untracked |
    ...
    Where-Object { $_ -and $_ -cnotmatch '^references/' } |
    Sort-Object -Unique)
...
foreach ($rel in $files) {
    ...
    foreach ($t in (Get-LinkTarget $text)) {
        ...
        # ⛔ COMPONENTS, NOT A REPEATED REGEX. A regex that globally removed
        # `name/../` also treated the first `..` in `../../` as a name and
        # collapsed a valid three-level traversal to the wrong directory.
        $parts = New-Object System.Collections.Generic.List[string]
        foreach ($part in ($joined -split '/')) {
            if (-not $part -or $part -eq '.') { continue }
            if ($part -eq '..') {
                if ($parts.Count -gt 0 -and $parts[$parts.Count - 1] -ne '..') {
