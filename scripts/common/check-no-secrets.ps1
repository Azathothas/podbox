...
if ($Public) {
    ...
    $hex = @(Find-Pattern '\b[0-9a-f]{24,}\b' |
        Where-Object { $_ -notmatch '^Cargo\.lock:' } |
        Where-Object { $_ -notmatch '^experiments/results/' } |
        Where-Object { $_ -notmatch 'uses:\s*[A-Za-z0-9._-]+/[A-Za-z0-9._-]+@[0-9a-f]{40}' } |
        Where-Object {
            $_ -notmatch 'sha256:|checksum|digest|commit|revision|tree identity|[A-Z0-9_]+_SHA256|[Pp]inned(Ref|Sha256|Commit|Digest)|PINNED_(REF|SHA256)'
        })
