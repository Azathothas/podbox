...
if [ -n "$SCOPE" ]; then
  list_files "$SCOPE" | grep '\.md$' | grep -v '^references/' > "$WORK/list" || true
else
  list_files '*.md' | grep -v '^references/' > "$WORK/list" || true
fi
...
awk -F"$TAB" '
  function normpath(path,   n, i, p, top, out) {
    n = split(path, p, "/"); top = 0
    for (i = 1; i <= n; i++) {
      if (p[i] == "" || p[i] == ".") continue
      if (p[i] == "..") {
        if (top > 0 && part[top] != "..") delete part[top--]
        else part[++top] = p[i]
      } else part[++top] = p[i]
    }
    out = ""
    for (i = 1; i <= top; i++) out = out (out == "" ? "" : "/") part[i]
    for (i in part) delete part[i]
    return out
  }
  $1 == "RAW" {
    tgt = $3
