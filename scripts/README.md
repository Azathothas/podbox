...
drifted the way an unchecked rule always drifts. Its first run over the
template tree found **42** duplicated sentences of 8 words or more, ⭐ five
of them in the template's rules skeleton. That template-only skeleton was not
imported here; it went from 198 lines to 134 and now links what it used to
copy.
...
⛔ **Entry-point routers are exempt from each other and from nothing else.**
The template has three routers because a session may be handed exactly one of
them. This repository deliberately imports only `AGENTS.md`; a sentence shared
between that router and any other file is still refused, which was verified by
planting one.
