...
# put that cost BEHIND the reading `AGENTS.md` opens with, and this is the
...
# `AGENTS.md` says the writable allowance is fixed and `df` misleads.
...
words=0
for f in TODO/PROGRESS.md AGENTS.md; do
	n=$(wc -w <"$REPO/$f")
	say "  $(printf '%-22s' "$f") $n words"
	words=$((words + n))
done
say "  together              $words words"
say "  ⭐ That is the reading AGENTS.md opens with, and it needs no"
