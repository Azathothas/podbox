#!/usr/bin/env bash
# Question: can podbox log in to a private registry and pull from it, with
# the credential never entering this tree's logs, errors, or result files?
#
# TODO/image.md T-0209.
#
# The fixture is T-0206's: one pinned zot binary plus a seeded storage dir, a
# generated cert and an htpasswd file, all staged host-side; the Prove runs
# inside one `--network=none` driver container where zot and podbox meet on
# container loopback. Only the htpasswd-required config serves here: the
# anonymous 401 with its Basic challenge is what podbox must answer, and the
# open half already proved podbox pulls anonymously (T-0206).
#
#   ./200-registry-auth.sh
#   PODBOX_BIN=/path/to/podbox ./200-registry-auth.sh
#
...
OUT="$REPO/experiments/results/registry-auth.txt"
case "$(uname -s)" in
Linux) NATIVE=1; WORK="$(mktemp -d)" ;;
*)     NATIVE=0; WORK="$REPO/experiments/.sweep200-work"; rm -rf "$WORK"; mkdir -p "$WORK/w" || exit 2 ;;
esac

# ⭐ THE PIN, shared with 180: one release binary, verified every run.
ZOT_VERSION="v2.1.21"
...
# into a container (the 180 shape).
...
# The 180 shape; this script stands alone, so the seeder rides along.
say "== seed"
seed_repo() {
	...
	_csize="$(wc -c <"$_d/blobs/sha256/config.tmp" | cut -d' ' -f1)" || return 1
...
pass "seeded $REPO_NAME:$REPO_TAG sha256:$SEED_REQ"
...
# which the EXIT trap removes, and the verdict asserts it is absent from
# the result file.
...
# spelling a native binary reads (the 180/280 shape), with conversion off
# for the call or -subj arrives as a path.
: >"$WORK/w/empty.cnf"
...
pass "required config written, cert carries SAN IP:127.0.0.1, htpasswd holds one user"
...
# ------------------------------------------------------- the driver script
# Written with literals baked in (digest, port, repo, user); the password is
# read from /w/.pass inside, never baked and never printed.
say "== driver run (network none)"
cat >"$WORK/w/driver.sh" <<DRIVER_EOF
#!/bin/sh
# 200 inside the none-network container: zot and podbox on loopback alone.
set -u
SEED_REQ="$SEED_REQ"
USER="$FIX_USER"
PORT_REQ="$REQ_PORT"
...
if /zb verify /w/config-required.json >/dev/null 2>&1; then ok "cfg-1: required config verifies"; else bad "cfg-1: required config rejected"; fi
...
if curl -sS --max-time 15 --cacert /w/server.cert -o /dev/null -D /w/hdrs -w "%{http_code}" "https://127.0.0.1:\$PORT_REQ/v2/" > /w/code0 2>/dev/null; then true; fi
if [ "\$(cat /w/code0)" = "401" ] && grep -qi 'www-authenticate: basic' /w/hdrs 2>/dev/null; then
	ok "req-1: GET /v2/ anon is 401 with a Basic challenge"
else
	bad "req-1: GET /v2/ anon is not a Basic 401"
fi

echo "-- login stores, pull sends (HOME_A)"
# ⛔ Homes live on tmpfs, not on the /w bind: the Windows-backed mount
# reports every mode as 777 whatever chmod says (measured 2026-09-22),
# so the 600 proof needs a filesystem with a mode bit.
export HOME=/dev/shm/h-a
mkdir -p "\$HOME" || { bad "setup: HOME_A"; }
PASS="\$(cat /w/.pass)"
if printf '%s' "\$PASS" | /pb login -u "\$USER" --password-stdin "127.0.0.1:\$PORT_REQ" >/w/login-a.log 2>&1; then
	if grep -q "Login Succeeded" /w/login-a.log; then
		ok "auth-1: login exits 0 and prints Login Succeeded"
	else
		bad "auth-1: login exits 0 without Login Succeeded"
	fi
else
	bad "auth-1: login failed"
fi
if [ "\$(stat -c %a "\$HOME/.docker/config.json" 2>/dev/null)" = "600" ]; then
	ok "auth-2: the stored file is owner-only (600)"
else
	bad "auth-2: the stored file is not 600"
fi
AUTH_LINE="\$(grep -o '\"auth\":\"[^\"]*\"' "\$HOME/.docker/config.json" 2>/dev/null | head -1 | cut -d'"' -f4)"
# ⛔ Single quotes above are the driver's: inside them a backslash is
# literal, so grep reads an escaped quote as a plain quote. `cut` takes
# no such quoting and wants the bare quote mark, which is why the two
# spellings differ on one line.
CRED="\$(printf '%s' "\$AUTH_LINE" | base64 -d 2>/dev/null)"
if [ -n "\$CRED" ] && [ "\${CRED%%:*}" = "\$USER" ]; then
	ok "auth-3: the stored entry reads back as the login user"
else
	bad "auth-3: the stored entry is not the login user"
fi
CRED=""; AUTH_LINE=""
# ⛔ One store per pull clause: a second pull of a held reference exits 0
# without touching the network, which would prove nothing about the login
# under test and would let a refusal clause pass without being refused.
if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs4 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" >/w/pull-tag.log 2>&1; then
	GOT="\$(SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs4 /pb inspect --format '{{.Digest}}' "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" 2>/dev/null)"
	if [ "\$GOT" = "sha256:\$SEED_REQ" ]; then
		ok "auth-4: authed pull by tag exits 0 and inspects to the seeded digest"
	else
		bad "auth-4: pulled digest \$GOT is not sha256:\$SEED_REQ"
	fi
else
	bad "auth-4: authed pull by tag failed"
fi
if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs5 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO@sha256:\$SEED_REQ" >/w/pull-digest.log 2>&1; then
	ok "auth-5: authed pull by digest exits 0"
else
	bad "auth-5: authed pull by digest failed"
fi
if printf '%s' "\$PASS" | /pb login -u "\$USER" --password-stdin "https://127.0.0.1:\$PORT_REQ/v2/" >/w/login-scheme.log 2>&1; then
	if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs6 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" >/w/pull-scheme.log 2>&1; then
		ok "auth-6: a schemeful server names the same host and still pulls"
	else
		bad "auth-6: pull after a schemeful login failed"
	fi
else
	bad "auth-6: schemeful login failed"
fi
if printf '%s' "wrong-password" | /pb login -u "\$USER" --password-stdin "127.0.0.1:\$PORT_REQ" >/w/login-wrong.log 2>&1; then
	if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs7 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" >/w/pull-wrong.log 2>&1; then
		bad "auth-7: a wrong-password pull succeeded"
	else
		if grep -qi '401' /w/pull-wrong.log 2>/dev/null; then
			ok "auth-7: a wrong-password pull fails naming 401"
		else
			bad "auth-7: a wrong-password pull fails without naming 401"
		fi
	fi
else
	bad "auth-7: wrong-password login itself failed"
fi

echo "-- refusals (fresh homes, no usable login)"
export HOME=/dev/shm/h-b
mkdir -p "\$HOME" || { bad "setup: HOME_B"; }
if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs8 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" >/w/pull-anon.log 2>&1; then
	bad "auth-8: an anonymous pull succeeded against a required registry"
else
	if grep -q -F "no login is stored" /w/pull-anon.log 2>/dev/null; then
		ok "auth-8: an anonymous pull fails naming the missing login"
	else
		bad "auth-8: an anonymous pull fails without naming the missing login"
	fi
fi
export HOME=/dev/shm/h-c
mkdir -p "\$HOME/.docker" || { bad "setup: HOME_C"; }
printf '{"auths":{},"credHelpers":{"127.0.0.1:%s":"podbox-no-such-helper-xyz"}}' "\$PORT_REQ" >"\$HOME/.docker/config.json"
if SSL_CERT_FILE=/w/server.cert PODBOX_STORE=/w/pbs9 /pb pull "127.0.0.1:\$PORT_REQ/\$REPO:\$TAG" >/w/pull-helper.log 2>&1; then
	bad "auth-9: a bogus-helper pull succeeded"
else
	if grep -q -F "podbox-no-such-helper-xyz" /w/pull-helper.log 2>/dev/null; then
		ok "auth-9: a bogus helper fails naming the helper"
	else
		bad "auth-9: a bogus helper fails without naming the helper"
	fi
fi

echo "-- usage shapes (no network)"
if printf '%s' "\$PASS" | /pb login -u "\$USER" >/dev/null 2>&1; then
	bad "auth-10: login without --password-stdin succeeded"
else
	if [ "\$?" = "125" ]; then
		ok "auth-10: login without --password-stdin is 125"
	else
		bad "auth-10: login without --password-stdin is not 125"
	fi
fi
if printf '%s' "\$PASS" | /pb login -u "\$USER" --password-stdin a.example b.example >/dev/null 2>&1; then
	bad "auth-11: login with two servers succeeded"
else
	if [ "\$?" = "1" ]; then
		ok "auth-11: login with two servers is 1"
	else
		bad "auth-11: login with two servers is not 1"
	fi
fi
