...
⭐ **The session record, saved beside [PROGRESS.md](../../../TODO/PROGRESS.md).** PROGRESS.md
...
| found by | what |
| --- | --- |
| building [T-0211](../../../TODO/image.md)'s own fix | `O_CLOEXEC` is close-on-**exec**; there is no close-on-**fork**, so the entry's stated fix could not work |
| the door sweep | [T-0212](../../../TODO/image.md) guarded `find_for` and left `find_one` open, so `extract` silently unpacked the wrong architecture |
| `scripts/dev.sh check`, first run | [T-0113](../../../TODO/probe.md): two probes in one process took each other's scratch name and `/tmp` read as unwritable |
...
| | |
| --- | --- |
| [T-1104](../../../TODO/milestones.md) is `partial` | [T-0505](../../../TODO/enter.md) `exec`, [T-0801](../../../TODO/cli.md) the parity table, [T-0803](../../../TODO/cli.md) the `docker` name |
| [T-0506](../../../TODO/enter.md) is `partial` | a probe inside a `qemu-user` container measures the emulator and is not yet labelled as such |
