...
| ⭐ a concurrent fork, as a condition | unmeasured | **necessary**, 0 of 20 in each of two passes | the same, clause 6 |
...
1. ⛔ **A CONTROL'S SKIP LIST IS A CLAIM ABOUT THE CODE, and this session's was
   ...
   ⭐ The last error was caught by a review pass that read the COMMAND LINE in
   the committed evidence instead of the label above it, and **completing the
   list reversed the verdict**: three red readings became 0 of 20 twice.
...
open.** What it now has: the series, one refuted mechanism, two measured
necessary conditions, the filesystem ruled out, and an instrument with a
positive control. What it does not have: the holder's name. ⭐ The entry's
`Approach` step 4b is the cheap next move the fork result justifies: eight of
the nine `Lock` sites take no `sys::close_in_children`, so register them and
re-run the subject, applied and measured and reverted if it changes nothing.
