...
⛔ **Removed from the template repository on 2026-08-30**: `deslop`,
`git-sync`, `fill-license` and `write-file.mjs`, both halves of each, plus
their Windows PowerShell wrapper directory. None of those removed paths is
imported here.
...
`scripts/common/mine-repo.sh`, on the operator's ruling: it encodes this
