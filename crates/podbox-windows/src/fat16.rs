...
const _: () = assert!(VOLUME_SECTORS - (DATA_SECTOR + CLUSTERS * SECTORS_PER_CLUSTER) < SECTORS_PER_CLUSTER);
...
impl Fat16 {
    ...
    fn read_chain(&self, first: u16, size: u32) -> Option<Vec<u8>> {
        ...
        let mut n = first;
        while n >= 2 && n < 0xFFF8 && out.len() < size as usize {
            let at = self.cluster_at(n);
...
fn parse_name(name: &str) -> Result<([u8; 11], u8), String> {
    ...
    let ok = |s: &str| {
        s.bytes().all(|c| {
            c.is_ascii_alphanumeric() || matches!(c, b'$' | b'%' | b'\'' | b'-' | b'_' | b'@' | b'~' | b'!' | b'#' | b'&' | b'(' | b')' | b'{' | b'}' | b'^')
        })
    };
    if !ok(base) || !ok(ext) {
...
mod tests {
    ...
    fn the_geometry_is_internally_consistent() {
        // The FAT has to hold one entry per cluster plus the two reserved.
        assert!(CLUSTERS + 2 <= FAT_SECTORS * SECTOR / 2);
        assert!(CLUSTERS + 2 < 0xFFF0);
        assert!(DATA_SECTOR + CLUSTERS * SECTORS_PER_CLUSTER <= VOLUME_SECTORS);
