...
fn run_steps(argv: &[String]) -> Result<(), Error> {
    let (prog, rest) = argv.split_first().ok_or_else(|| Error::Io("empty argv".into()))?;
...
pub fn provision(
    ...
    while start.elapsed() < wait {
        if child.try_wait().map_err(|e| Error::Io(e.to_string()))?.is_some() {
            break;
        }
...
/// Type `text` into the guest console through the emulator's monitor socket.
fn type_into_console(plan: &Plan, text: &str) -> Result<(), Error> {
    let mut sock = connect_monitor(&plan.monitor, Duration::from_secs(60))?;
    for ch in text.chars() {
        let k = keys(ch).ok_or_else(|| Error::Io(format!("no key for {ch:?}")))?;
        let name = if k.1 { format!("shift-{}", k.0) } else { k.0.to_string() };
        writeln!(sock, "sendkey {name} 40").map_err(|e| Error::Io(e.to_string()))?;
        std::thread::sleep(Duration::from_millis(8));
    }
    writeln!(sock, "sendkey ret 80").map_err(|e| Error::Io(e.to_string()))?;
    std::thread::sleep(Duration::from_millis(500));
    Ok(())
}

fn connect_monitor(path: &Path, wait: Duration) -> Result<UnixStream, Error> {
    let start = Instant::now();
    loop {
        ...
        if start.elapsed() >= wait {
            return Err(Error::Io(format!(
                "the monitor socket {} never appeared",
                path.display()
            )));
        }
...
/// Read every byte the monitor has to say, so `quit` is delivered rather
/// than the connection closing under it.
pub fn stop(plan: &Plan) {
    if let Ok(mut s) = UnixStream::connect(&plan.monitor) {
