...
fn type_into_console(plan: &Plan, text: &str) -> Result<(), Error> {
    let mut m = qmp::Monitor::connect(&plan.monitor, Duration::from_secs(120)).map_err(Error::Io)?;
    for ch in text.chars() {
        let (name, shifted) =
            keys(ch).ok_or_else(|| Error::Io(format!("no key for {ch:?}")))?;
        if shifted {
...
mod tests {
    ...
    fn cleanup_never_removes_the_directory_a_vendor_image_lives_in() {
        ...
        cleanup(&not_ours);
        assert!(vendor.is_dir(), "an unrecognised directory is never removed");
