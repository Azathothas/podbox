...
pub struct Plan {
    ...
    pub serial: PathBuf,
    /// The `unix` monitor socket, which is how provisioning types into the
    /// console and how a stuck run is stopped.
    pub monitor: PathBuf,
    /// Guest memory in MiB.
    pub memory_mib: u64,
    pub cpus: u32,
...
pub fn argv(p: &Plan) -> Vec<String> {
    ...
    push("-monitor");
    push(&format!("unix:{},server,nowait", p.monitor.display()));
