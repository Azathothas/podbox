...
pub(crate) fn parse(args: &[String]) -> Result<Option<Args>, i32> {
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            ...
            "--podbox-qemu-arg" => a.emu_args.push(value(&mut it, "--podbox-qemu-arg")?.clone()),
            "-h" | "--help" => {
                print!("{USAGE}");
                return Ok(None);
            }
...
mod tests {
    ...
    fn no_subcommand_and_an_unknown_flag_are_flag_errors() {
        ...
        assert!(p(&["wat"]).is_ok(), "an unknown subcommand parses; dispatch refuses it");
    }
    ...
    fn a_sha256_that_is_not_a_digest_is_refused() {
        ...
        let a = p(&["fetch", "--url", "u", "--sha256", &"A".repeat(64)])
            .unwrap()
            .unwrap();
        assert_eq!(a.sha256, Some("a".repeat(64)), "pins normalise to lowercase");
    }
